defmodule Explorer.GasPrice.History.Cataloger do
  @moduledoc """
  ...
  @impl GenServer
  def init(:ok) do
    send(self(), {:fetch_gas_price})

    {:ok, %{}}
  end

  @impl GenServer
  def handle_info({:fetch_gas_price}, state) do
    fetch_gas_price()
  ...
  def handle_info({_ref, {_, {:ok, records}}}, state) do
    ...
    Process.send_after(self(), {:fetch_gas_price}, fetch_after)
  ...
  @impl GenServer
  def handle_info({_ref, {failed_attempts, :error}}, state) do
    Logger.warn(fn -> "Failed to fetch market history. Trying again." end)

    fetch_gas_price(failed_attempts + 1)
  ...
  @spec source() :: module()
  defp source do
    config_or_default(:source, Explorer.GasPrice.History.Source.GetGasPrice)
  end

  @spec fetch_gas_price( non_neg_integer()) :: Task.t()
  defp fetch_gas_price(failed_attempts \\ 0) do
    Task.Supervisor.async_nolink(Explorer.GasPriceTaskSupervisor, fn ->
      Process.sleep(delay(failed_attempts))
      {failed_attempts, source().fetch_gas_price()}
    end)
  end
