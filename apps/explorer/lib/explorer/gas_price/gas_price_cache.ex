defmodule Explorer.GasPrice.GasPriceHistoryCache do
  @moduledoc """
  Caches recent gas price history.
  """

  import Ecto.Query, only: [from: 2]

  alias Explorer.GasPrice.GasPriceHistory
  alias Explorer.Repo

  @cache_name :gas_price_history
  @last_update_key :last_update
  @history_key :history
  # 6 hours
  @recent_days 30

  def fetch do
    if cache_expired?() do
      update_cache()
    else
      fetch_from_cache(@history_key)
    end
  end

  def cache_name, do: @cache_name

  def data_key, do: @history_key

  def updated_at_key, do: @last_update_key

  def recent_days_count, do: @recent_days

  defp cache_expired? do
    cache_period = gas_price_history_cache_period()
    updated_at = fetch_from_cache(@last_update_key)

    cond do
      is_nil(updated_at) -> true
      current_time() - updated_at > cache_period -> true
      true -> false
    end
  end

  defp update_cache do
    new_data = fetch_from_db()

    put_into_cache(@last_update_key, current_time())
    put_into_cache(@history_key, new_data)

    new_data
  end

  defp fetch_from_db do
    day_diff = @recent_days * -1

    query =
      from(
        mh in GasPriceHistory,
        where: mh.date > date_add(^Date.utc_today(), ^day_diff, "day"),
        order_by: [desc: mh.date]
      )

    Repo.all(query)
  end

  defp fetch_from_cache(key) do
    ConCache.get(@cache_name, key)
  end

  defp put_into_cache(key, value) do
    ConCache.put(@cache_name, key, value)
  end

  defp current_time do
    utc_now = DateTime.utc_now()

    DateTime.to_unix(utc_now, :millisecond)
  end

  defp gas_price_history_cache_period do
    case Integer.parse(System.get_env("GAS_PRICE_HISTORY_CACHE_PERIOD", "")) do
      {secs, ""} -> :timer.seconds(secs)
      _ -> :timer.minutes(1)
    end
  end
end
